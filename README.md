...
- **🆕 分割実行**: ドライブ一覧取得とファイル取得を分離（v2.0）
- **🆕 段階的処理**: 1ドライブずつ手動実行で大規模環境に対応（v2.0）
- **エラー処理**: 堅牢なエラーハンドリング
...
- **Google Workspace 特権管理者権限**（必須）
  - `useDomainAdminAccess` オプションを使用するため、特権管理者（スーパー管理者）権限が必要
  - または「ドライブとドキュメントの設定」権限を持つカスタム管理者ロール
- **調査対象の共有ドライブへのメンバー権限**
  - メンバーでない共有ドライブは一覧に表示されますが、中身のファイルは取得できません
  - ファイル情報を取得するには「閲覧者」以上の権限で追加される必要があります
...
#### 🆕 方法A: 分割実行（推奨 - 大規模環境向け）
```javascript
// Step 1: 共有ドライブ一覧取得（1回のみ）
driveGet()
→ 00_共有ドライブ一覧シート作成

// Step 2: ファイル情報取得（繰り返し実行）
fileGet()  // 1回目: 最初のドライブを処理
fileGet()  // 2回目: 次のドライブを処理
fileGet()  // 3回目: ...
// 全ドライブ分繰り返す
```

**メリット:**
- ✅ 6分制限を回避（1ドライブずつ確実に処理）
- ✅ 途中でエラーが出ても他のドライブに影響なし
- ✅ 進捗状況が明確

#### 方法B: 一括実行（従来方式 - 小規模環境のみ）
```javascript
main()  // 非推奨: v1.x互換
```

**注意:** 共有ドライブが多い場合は6分制限で停止する可能性があります

### 4. 結果確認
- 結果: 「Drive情報収集結果」スプレッドシート
- マスターシート: `00_共有ドライブ一覧`
- 個別シート: `01_ドライブ名`, `02_ドライブ名`, ...
...
**列構成（全18列）:**
```
レベル | パス | 種別 | 名前 | ID | 親ID | 作成者 | 作成日 | 更新日 | サイズ |
管理者 | コンテンツ管理者 | 投稿者 | 編集者 | 閲覧者（コメント可） | 閲覧者 | 外部共有 | URL
```
...
| 列名 | フォルダの場合 | ファイルの場合 |
|-----|-------------|--------------|
| 管理者 | 上位権限 + 個別権限 | 上位権限のみ |
| コンテンツ管理者 | 上位権限 + 個別権限 | 上位権限のみ |
| 投稿者 | 上位権限 + 個別権限 | 上位権限のみ |
| 編集者 | 「ー」（編集者の概念なし） | 個別に設定された編集者のみ |
| 閲覧者（コメント可） | 上位権限 + 個別権限 | 上位権限 + 個別権限 |
| 閲覧者 | 上位権限 + 個別権限 | 上位権限 + 個別権限 |

**表示例:**
```
レベル | パス              | 種別     | 名前         | 管理者           | コンテンツ管理者      | 投稿者          | 編集者          | 外部共有
0      | /                 | フォルダ | ルート        | 山田太郎:admin@  | 田中次郎:manager@    |                 | ー              | 組織内共有あり
1      | /営業資料/         | フォルダ | 営業資料      | 山田太郎:admin@  | 田中次郎:manager@    | 佐藤花子:sales@ | ー              | 組織内共有あり
2      | /営業資料/提案書   | ファイル | 提案書.xlsx   | 山田太郎:admin@  | 田中次郎:manager@    | 佐藤花子:sales@ | 鈴木一郎:user@  | 外部共有あり(1件)
```

**重要:**
- **上位権限**: 共有ドライブレベルで設定された権限が全てのファイル・フォルダに自動継承されます
- **個別権限**: 特定のフォルダ・ファイルに直接追加された権限です
- **重複除去**: 同じユーザーが上位権限と個別権限の両方を持つ場合、1回のみ表示されます
...
- **全共有ドライブの一覧取得**
  - 組織内の全ての共有ドライブを検出（メンバーでなくても）
  - 共有ドライブ名、ID、作成日などの基本情報を取得
...
- **メンバーでない共有ドライブの中身の取得**
  - 共有ドライブ内のフォルダ・ファイル情報
  - 権限設定や外部共有状況
  - **ドメイン管理者でも、メンバーでない共有ドライブの中身は見えません**
...
- ✅ 配下の**全てのフォルダ**にアクセス可能
- ✅ 配下の**全てのファイル**にアクセス可能
- ✅ 個別のフォルダ・ファイルに権限設定がなくても自動的に継承
- ✅ 共有ドライブより厳しい権限は設定できない仕様
...
↓
/営業資料/            → 閲覧可能 (権限継承)
/営業資料/2024年/    → 閲覧可能 (権限継承)
/営業資料/.../提案書.xlsx → 閲覧可能 (権限継承)
```

#### ⚠️ 例外ケース

稀に、特定のファイルに「より高い権限」が個別設定されている場合がありますが、それでもメンバーなら閲覧可能です。

### 本番運用の推奨手順

1. **調査対象の共有ドライブを特定**
2. **各共有ドライブに実行ユーザーを「閲覧者」として追加**
   - Google Drive UI → 共有ドライブ → メンバーを管理
3. **ツールを実行**
4. **調査完了後、必要に応じてメンバーから削除**
...
### アクセス制限
- **メンバー権限が必要**: 共有ドライブのメンバーでない場合、中身のファイルは取得できません
- **特権管理者権限が必要**: `useDomainAdminAccess` を使用するため
- **権限継承**: 共有ドライブのメンバーになれば、配下の全てのフォルダ・ファイルに自動的にアクセス可能
...
| バージョン | 日付 | 変更内容 |
|-----------|------|----------|
| 2.0 | 2025/10/02 | **大規模環境対応**: driveGet()とfileGet()による分割実行機能追加、1ドライブずつ手動実行可能に、6分制限回避 |
| 1.3 | 2025/09/30 | 権限表示の大幅改善：「管理者」列追加、フォルダ・ファイルで上位権限と個別権限を適切に表示、重複除去機能実装 |
| 1.2 | 2025/09/30 | useDomainAdminAccess オプション追加（全共有ドライブ・全ファイル・全権限情報へのアクセス対応） |
| 1.1 | 2025/09/30 | 権限取得機能強化（Drive.Permissions.list使用）、組織内/外部共有の区別表示、統計情報収集、OAuth スコープ最適化 |
